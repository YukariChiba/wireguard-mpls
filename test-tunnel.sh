...
sudo ip route add ${NS_MPLS_IP}/32 encap mpls $MPLS_LB_HOST via ${NS_WG_IP} src ${HOST_MPLS_IP}
sudo ip netns exec ${NETNS} ip route add ${HOST_MPLS_IP}/32 encap mpls $MPLS_LB_NS via ${HOST_WG_IP} src ${NS_MPLS_IP}
...
sudo ip -M route add 101 as 102 via inet ${HOST_WG_IP} dev ${NIC}-wg
sudo ip netns exec ${NETNS} ip -M route add 102 dev lo

sudo ip -M route add 200 as 201 via inet ${NS_WG_IP} dev ${NIC}-wg
sudo ip netns exec ${NETNS} ip -M route add 201 as 202 via inet ${HOST_WG_IP} dev ${NIC}-wg
sudo ip -M route add 202 dev lo
